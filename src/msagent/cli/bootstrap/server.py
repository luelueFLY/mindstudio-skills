...
from msagent.cli.bootstrap.webapp import app
...
MSAGENT_ROOT = Path(__file__).resolve().parents[4]
...
def generate_langgraph_json(working_dir: Path) -> None:
    ...
    config = {
        "dependencies": [str(MSAGENT_ROOT)],
        # Path is relative to dependency root (project root). Keep src/ prefix for editable installs.
        "graphs": {"agent": "src/msagent/cli/bootstrap/server.py:get_graph"},
        "http": {"app": "src/msagent/cli/bootstrap/server.py:app"},
    }
    ...
    # Write to .msagent/langgraph.json
...
async def handle_server_command(args) -> int:
    ...
    try:
        ...
        # Load agent config for the server session
        agent_config = await initializer.load_agent_config(args.agent, working_dir)
        if args.model:
            await initializer.load_llm_config(args.model, working_dir)
        ...
        env = os.environ.copy()
        env["MSAGENT_WORKING_DIR"] = str(working_dir)
        if args.agent:
            env["MSAGENT_AGENT"] = args.agent
        if args.model:
            env["MSAGENT_MODEL"] = args.model

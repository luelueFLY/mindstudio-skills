...
async def lifespan(app: FastAPI):
    ...
    agent = os.getenv("MSAGENT_AGENT")
    model = os.getenv("MSAGENT_MODEL")
    working_dir_str = os.getenv("MSAGENT_WORKING_DIR")

    if not working_dir_str:
        raise ValueError("MSAGENT_WORKING_DIR environment variable is required")
